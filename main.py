...
def main(city=None, local_path: str=None, db_config: dict=None, tag_local=True, tag_db=False, 
         model_path=None, house_district=False, multi_process=False, price_low=None, price_high=None, 
         *args, **kwargs):
    """
    ...
    if not multi_process:
        ...
        if not city:
            ...
            logger.info('== 开始获取链家的房源信息 {} =='.format(time.asctime()))
            info_lj = RoomInfoCatchingLJ.init_city(city, price_low, price_high).get_room_info_total()
            # logger.info('== 链家房源信息获取完毕，开始获取自如房源信息 {} =='.format(time.asctime()))
            # info_zr = RoomInfoCatchingZR.init_city(city).get_room_info_total()
        
        # logger.info('== 自如房源信息获取完毕 {} =='.format(time.asctime()))
        # if not model_path:
        #     model_path = os.path.join(path_root, r'utils\ocr\pre_trained_model\LR_0906.pickle')
        # # 添加价格
...
def make_argsparse():
    ...
    parse.add_argument('--city', default='杭州', type=str)  # 需要访问的城市名, 如未输入，默认杭州
    ...
    parse.add_argument('--house_district', default=True, action='store_true') # 是否计算小区数据   
    ...
    parse.add_argument('--price_low', default='3000', type=str) # 租金最低价
    parse.add_argument('--price_high', default='6000', type=str) # 租金最高价    
